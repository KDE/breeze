...
bool Style::drawWidgetPrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    Q_UNUSED(option)
    auto parent = widget;
    if (!_toolsAreaManager->hasHeaderColors() || !_helper->windowContentsAreEdgeToEdge(widget)) {
        return true;
    }
...
bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (widget && widget->property(PropertyNames::bordersSides).isValid()) {
        ...
        if (_helper->windowContentsAreEdgeToEdge(widget)) for (auto& [edge, method] : edges) {
            if (!(sides & edge))
                continue;
        ...
        if (_helper->windowContentsAreEdgeToEdge(widget) && sides & Qt::TopEdge) {
